...
export default async function ({ octokit, workflow_id, run_id, before, crucial_jobs }) {
  ...
  const waiting_for = workflow_runs
    ...
    .filter(run => ['in_progress', 'queued', 'waiting', 'pending'].includes(run.status))
    // exclude this one
    ...
    .filter(async (run) => {
      ...
      const crucialJobs = jobs.filter(job => ['in_progress','waiting','queued'].includes(job.status) && crucialJobNames.find(name => job.name.startsWith(name)));
